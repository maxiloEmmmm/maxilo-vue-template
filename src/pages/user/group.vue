<template>
    <mxl-row-col-with-wrapper>
        <mxl-card ref="treeLoading">
            ...
            <mxl-tree
                ...
                <mxl-list-in-line slot="example-slot" slot-scope="props">
                    <mxl-emotion-text type="info">管理数: {{ props.treeNode.roles.length }}</mxl-emotion-text>
                    ...
                    <mxl-btn-group>
                        ...
                        <mxl-btn @click="refs(props.treeNode)"><i class="fa fa-users"></i></mxl-btn>
                    </mxl-btn-group>
                </mxl-list-in-line>
            </mxl-tree>
        </mxl-card>
        ...
        <mxl-modal :show.sync="modal.user" ref="userModal" :loading="loading.userModal">
            <mxl-row>
                <mxl-cols cols="5-2-5">
                    <mxl-table :useCheck="true" :ths="_group_user_ths" :url="_api_user"  :init="false" ref="userTable"></mxl-table>
                    <mxl-list-group align="center">
                        <mxl-btn @click="userStore('userTable')">&gt;</mxl-btn>
                        <mxl-btn @click="userStore('userExceptTable')">&lt;</mxl-btn>
                    </mxl-list-group>
                    <mxl-table :useCheck="true" :ths="_group_user_ths" :url="_api_user_except" :init="false" ref="userExceptTable"></mxl-table>
                </mxl-cols>
            </mxl-row>
        </mxl-modal>
    </mxl-row-col-with-wrapper>
...
export default {
    ...
    methods: {
        ...
        async refs(treeNode){
            this.$set(this.ds, 'opear_group_id', treeNode.id);
            await this.$nextTick();
        ...
        async userStore(ref){
            let users = this.$refs[ref].getCheckData('id', true);
            if(users.length === 0) {
                this.$utils.diy.alert.base({text: '请至少选择一个用户!', icon: 'warning'});
            }else {
                if(await this.$utils.diy.alert.confirm('确认' + ((ref=='userTable')?'移除':'加入') + '?' )) {
                    this.$refs.userModal.$refs.loading.mxlLoadingInstance.wait(async () => {
                        await this.$utils.async.alert(await this.$http.put(this.api.USER, {group_id: this.ds.opear_group_id, users}));
                        this.$refs.userTable.refresh();
                        this.$refs.userExceptTable.refresh();
                    });
                }
    ...
    computed: {
        ...
        _api_user(){
            return this.api.INDEX + '/' + this.ds.opear_group_id + '/user';
        },
        _api_user_except(){
